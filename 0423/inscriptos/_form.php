 ...
 <?php
  ...
  if (isset($_GET['error']) && $_GET['error'] == 1) {
    ...
    if (isset($_GET['error']) && $_GET['error'] == 3) {
      echo "<p style='color:red;'>El usuario ya está inscripto en este evento.</p>";
    }
  //$target="guardar.php";
  ?>
 <?php
 ...
 <form action="<?php echo $target; ?>" method="post">
   ...
   <input type="hidden" name="eventos_id" value="<?php echo isset($fila["eventos_id"]) ? $fila["eventos_id"] : ''; ?>">  <br><br>
   ...
   <input type="hidden" value='<?php echo $fila["fecha_checking"]; ?>' id="fecha_checking" name="fecha_checking" required class="form-control"><br><br>
   ...
   <?php if ($accion == "editar") { ?>
     ...
     <label for="checking">Presente:</label><br>
     <select id="checking" name="checking" required class="form-control">
       <option value="0" <?php echo ($fila["checking"] == 0) ? "selected" : ""; ?>>Ausente</option>
       <option value="1" <?php echo ($fila["checking"] == 1) ? "selected" : ""; ?>>Presente</option>
     </select><br>
   <?php } else { ?>
     <input type="hidden" name="estado" value="<?php echo $fila["estado"]; ?>">
     <input type="hidden" name="checking" value="<?php echo $fila["checking"]; ?>">
   <?php } ?>
   ...
   <?php } ?>
   <?php if ($fila["fecha_checking"] != "") { ?>
     <p>Fecha de check-in: <br> <span class="form-control"><?php echo $fila["fecha_checking"]; ?></span></p>
   <?php } ?>
