...
<div class="table-responsive">
    <table class="table table-striped table-bordered align-middle">
        ...
        <tr>
            ...
            <td><?php echo ($fila['estado'] == 0) ? 'Pendiente' : ''; ?>
                <?php echo ($fila['estado'] == 1) ? 'Confirmado' : ''; ?>
                <?php echo ($fila['estado'] == 2) ? 'Cancelado' : ''; ?>
            </td>
            <td><?php echo $fila['checking'] == 1 ? 'Presente' : 'Ausente'; ?></td>
            <td><?php echo $fila['fecha_inscripcion']; ?></td>
            <td><?php echo $fila['fecha_checking']; ?></td>
            <td>
                <a href="editar.php?id=<?php echo $fila['id']; ?>&eventos_id=<?php echo $ev_id; ?>" class="btn btn-outline-warning btn-sm">Editar</a>
            </td>
            <td>
                <a href="borrar.php?id=<?php echo $fila['id']; ?>&eventos_id=<?php echo $ev_id; ?>" class="btn btn-outline-danger btn-sm" onclick="return confirm('¿Seguro que quieres eliminar esta inscripción?');">Borrar</a>
            </td>
        </tr>
        <?php } ?>
    </table>
